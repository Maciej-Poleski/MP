...
private:
    ...
    void checkHeap() {
        ...
        for(size_t i = 1; i < _size; ++i) {
            if(_minTree[(i - 1) / 2] > _minTree[i]) {
                for(size_t i = 0; i < _size; ++i)
                    std::cout << _minTree[i].value << ' ';
                std::cout << '\n';
                std::cout << "Struktura kopca jest niepoprawna: _minTree[(i-1)/2]="
                          << _minTree[(i - 1) / 2].value << " _minTree[i]=" <<
                          _minTree[i].value << '\n';
                std::abort();
            }
        }
        ...
        for(size_t i = 0; i < _size; ++i) {
            if(_minTree[i].value != _maxTree[_minTree[i].link].value) {
                std::cout << "Dowiązanie z minimum " << _minTree[i].value << " do maksimum"
                          " jest niepoprawne :" << _maxTree[_minTree[i].link].value << '\n';
                for(size_t i = 0; i < _size; ++i)
                    std::cout << _minTree[i].value << ' ';
                std::cout << '\n';
                for(size_t i = 0; i < _size; ++i)
                    std::cout << _maxTree[i].value << ' ';
                std::cout << '\n';
                std::abort();
            }
...
int main(int argc, char **argv)
{
    ...
    vector<uint32_t> backend;
    mt19937 engine(404);
    uniform_int_distribution<uint8_t> mode(0, 2);
    uniform_int_distribution<uint32_t> value(0, 10);
    auto modeGenerator = bind(mode, engine);
    auto valueGenerator = bind(value, engine);
    bool stop = false;
    uint64_t iteration = 0;
    for(uint8_t m = modeGenerator(); !stop; m = modeGenerator(), ++iteration) {
        if(backend.empty() || m == 0) {
            uint32_t v = valueGenerator();
            heap.push(v);
            backend.push_back(v);
            //cout << "INSERT " << v << ' ' << heap.size() << '\n';
        } else if(m == 1) {
            uint32_t v1 = heap.popMin();
            auto v2i = min_element(backend.begin(), backend.end());
            uint32_t v2 = *v2i;
            if(v1 != v2) {
                cout << "Oczekiwano " << v2 << " a otrzymano " << v1 << '\n';
                stop = true;
            }
            backend.erase(v2i);
            //cout << "POP MIN " << v1 << ' ' << v2 << '\n';
        } else if(m == 2) {
            uint32_t v1 = heap.popMax();
            auto v2i = max_element(backend.begin(), backend.end());
            uint32_t v2 = *v2i;
            if(v1 != v2) {
                cout << "Oczekiwano " << v2 << " a otrzymano " << v1 << '\n';
                stop = true;
            ...
            cout << "Błąd wewnętrzny\n";
            stop = true;
        }
    }
    cout << "Wykonano iteracji: " << iteration << '\n';
}
