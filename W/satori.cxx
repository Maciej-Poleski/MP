...
static uint8_t *bestSolution;
static uint_fast32_t bestSolutionTime;
static uint8_t *currentSolution;
static uint8_t *currentSolutionTop;
static uint_fast32_t currentSolutionTimeA;
static uint_fast32_t currentSolutionTimeB;
static uint_fast32_t currentSolutionTimeC;

/**
 * Szacuje minimalny koszt dokończenia produkcji przy założeniu że wybrano
 * @c selected przedmiot jako następny.
 */
static uint_fast32_t
getBestEstimation(const Item *items, uint8_t size, uint8_t selected)
__attribute__((nonnull, pure, nothrow, hot));
static uint_fast32_t
getBestEstimation(const Item *items, uint8_t size, uint8_t selected)
{
    uint_fast32_t a = currentSolutionTimeA;
    for(size_t i = 0; i < size; ++i)
        a += items[i].a;
    uint_fast32_t b = std::max(currentSolutionTimeB,
                               currentSolutionTimeA + items[selected].a);
    for(size_t i = 0; i < size; ++i)
        b += items[i].b;
    uint_fast32_t c =
        std::max(currentSolutionTimeC,
                 std::max(currentSolutionTimeB,
                          currentSolutionTimeA + items[selected].a) +
                 items[selected].b
                );
    for(size_t i = 0; i < size; ++i)
        c += items[i].c;
    uint_fast32_t minB = std::numeric_limits< uint_fast32_t >::max();
    for(size_t i = 0; i < size; ++i)
        if(minB > (items[i].b + items[i].c) && i != selected)
            minB = items[i].b + items[i].c;
    uint_fast32_t minC = std::numeric_limits< uint_fast32_t >::max();
    for(size_t i = 0; i < size; ++i)
        if(minC > items[i].c && i != selected)
            minC = items[i].c;
    a += minB;
    b += minC;
    if(a >= b && a >= c)
...
struct Selection {
    uint8_t selected;
    uint32_t bestEstimation;
};

static bool operator<(const Selection &lhs, const Selection &rhs)
__attribute__((nonnull, nothrow, pure));
static bool operator<(const Selection &lhs, const Selection &rhs)
{
    return lhs.bestEstimation < rhs.bestEstimation;
}

static void findSolution(Item *items, uint8_t size)
__attribute__((nonnull, nothrow, hot));
static void findSolution(Item *items, uint8_t size)
{
    using std::swap;
    if(size == 0) {
        if(bestSolutionTime > currentSolutionTimeC) {
            bestSolutionTime = currentSolutionTimeC;
            memcpy(bestSolution, currentSolution, n * sizeof(uint8_t));
        }
    } else {
        Selection *selectionOrder = new Selection[size];
        for(size_t i = 0; i < size; ++i)
            selectionOrder[i] = {i, getBestEstimation(items, size, i)};
        insertionSort(selectionOrder, size);
        Item *newItems = new Item[size - 1];
        for(size_t i = 0; i < size; ++i) {
            size_t j = i + 1;
            while(j < size && (selectionOrder[i].bestEstimation ==
                               selectionOrder[j].bestEstimation))
                ++j;
            size_t newI = j;
            while(i < j)
                swap(selectionOrder[i++], selectionOrder[--j]);
            i = newI;
        }
        for(size_t i = 0; i < size; ++i) {
            if(selectionOrder[i].bestEstimation >= bestSolutionTime)
                break;
            Item *newItemsPtr = newItems;
            for(size_t j = 0; j < size; ++j)
                if(j != i)
                    *newItemsPtr++ = items[selectionOrder[j].selected];
            *currentSolutionTop++ = items[selectionOrder[i].selected].id;
            uint32_t oldA = currentSolutionTimeA;
            currentSolutionTimeA += items[selectionOrder[i].selected].a;
            uint32_t oldB = currentSolutionTimeB;
            currentSolutionTimeB =
                std::max(currentSolutionTimeA, currentSolutionTimeB) +
                items[selectionOrder[i].selected].b;
            uint32_t oldC = currentSolutionTimeC;
            currentSolutionTimeC =
                std::max(currentSolutionTimeB, currentSolutionTimeC) +
                items[selectionOrder[i].selected].c;
            findSolution(newItems, size - 1);
            --currentSolutionTop;
            currentSolutionTimeA = oldA;
            currentSolutionTimeB = oldB;
            currentSolutionTimeC = oldC;
        }
        delete [] newItems;
...
inline static void solution()
{
    ...
    while(z--) {
        ...
        for(size_t i = 0; i < n; ++i)
            in >> items[i].a >> items[i].b >> items[i].c;
        for(size_t i = 0; i < n; ++i)
            items[i].id = i + 1;
        bestSolution = new uint8_t[n];
        bestSolutionTime = std::numeric_limits< uint_fast32_t >::max();
        currentSolution = new uint8_t[n];
        currentSolutionTop = currentSolution;
        currentSolutionTimeA = 0;
        currentSolutionTimeB = 0;
        currentSolutionTimeC = 0;
        findSolution(items, n);
        out << bestSolutionTime << '\n';
        for(size_t i = 0; i < n; ++i)
            out << static_cast<uint_fast16_t>(bestSolution[i]) << ' ';
        out << '\n';
