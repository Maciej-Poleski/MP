...
inline static void solution()
{
    ...
    while(z--) {
        ...
        Event *ePtr = events;
        IntTree::Builder treeBuilder(n * 2);
        for(size_t i = 0; i < n; ++i) {
            int32_t x1, y1, x2, y2;
            in >> x1 >> y1 >> x2 >> y2;
            treeBuilder.insertPoint(y1);
            treeBuilder.insertPoint(y2);
            *ePtr++ = Event(x1, y1, y2, 0);
            *ePtr++ = Event(x2, y1, y2, 1);
        }
        IntTree intTree = treeBuilder.build();
        std::sort(events, ePtr);
        Event *eEnd = ePtr;
        ePtr = events;
        int_fast32_t k;
        in >> k;
        while(k--) {
            ...
            in >> x >> y;
            while((ePtr != eEnd) &&
                    ((ePtr->x < x) || ((ePtr->x == x) && (ePtr->p == 0)))) {
                switch(ePtr->p) {
                case 0:
                    intTree.add(ePtr->y1, ePtr->y2);
                    break;
                default:
                    check(ePtr->p == 1);
                    intTree.sub(ePtr->y1, ePtr->y2);
                    break;
                }
                ++ePtr;
            }
