...
struct Edge {
    ...
    Edge() {}
    Edge(uint32_t a, uint32_t b, uint16_t s, uint16_t length) :
...
inline static void solution()
{
    ...
    while(z--) {
        ...
        if(length <= value) {
            ...
            if(E[path[selectedState]].a == E[path[(selectedState + 1) % (2 * n)]].a ||
                    E[path[selectedState]].a == E[path[(selectedState + 1) % (2 * n)]].b) {
                out << E[path[selectedState]].a << '\n';
            } else {
                out << E[path[selectedState]].b << '\n';
            }
