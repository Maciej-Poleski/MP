...
inline static void solution()
{
    ...
    in >> z;
    while(z--) {
        uint_fast32_t n, a, b, r;
        in >> n >> a >> b >> r;
        uint_fast32_t R = 2 * r;
        uint32_t *D = new uint32_t[n];
        D[0] = 0;
        ...
        uint64_t *NT = new uint64_t[n];
        int32_t *LT = new int32_t[n];
        ...
        LT[0] = -1;
        for(size_t i = 1; i < n; ++i) {
            T[i] = std::min(T[i - 1], NT[i - 1]) + a;
            for(; leftT < i; ++leftT) {
                if(D[i] - D[leftT] <= R)
                    break;
            }
            uint64_t S1 = T[i - 1] + b;
            uint64_t S2 = NT[i - 1] + b;
            uint64_t S3 = (leftT < i) ? T[leftT] : std::numeric_limits< uint64_t>::max();
            if(S1 <= S2 && S1 <= S3) {
                NT[i] = S1;
                LT[i] = i - 1;
            } else if(S2 <= S1 && S2 <= S3) {
        ...
        for(; i >= 0;) {
            if(B[i]) {
                A[Ai++] = std::min<uint_fast32_t>(D[i] + r, D[n - 1]);
                --i;
            } else {
                i = LT[i];
                if(i >= 0)
